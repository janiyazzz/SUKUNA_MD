    ...
    async execute({ sock, msg, from, sender, reply, args, isGroup, isAdmin, isOwner, phoneNumber }) {
        ...
        if (sub === 'preview' || sub === 'test') {
            try {
                ...
                // Use the sender as the demo participant so the mention works.
                const demo = sender || (sock.user?.id) || from;
                await sendIntroCard(sock, from, demo, meta, grp);
            } catch (e) {
...
async function sendIntroCard(sock, groupId, participant, meta, grp) {
    ...
    // Use GROUP profile picture (not user's) — best effort.
    const withTimeout = (pr, ms) => Promise.race([
        pr,
        new Promise((_, rej) => setTimeout(() => rej(new Error('pp timeout')), ms))
    ]);
    let gpicUrl = null;
    try { gpicUrl = await withTimeout(sock.profilePictureUrl(groupId, 'image'), 4000); } catch (_) {}
    ...
    if (gpicUrl) {
        try {
            ...
            console.warn('[introcard] image send failed, falling back to text:', e.message);
        }
    }
    try {
        ...
        // Surface in-group so admins know the card tried to fire.
        try {
            await sock.sendMessage(groupId, {
                text: `⚠️ Intro card couldn't be sent: ${e.message}\n_Check that the bot has permission to send messages here._`,
            });
        } catch (_) {}
        throw e;
    }
