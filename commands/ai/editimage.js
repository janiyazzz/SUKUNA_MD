 ...
 *  1. David Cyril "NanoBanana" img2img — free, no key, real img2img via
 *     Google's Gemini "Nano Banana" model (apis.davidcyril.name.ng).
 *  2. prexzyvilla img2img                — fallback, real img2img
 *  3. Pollinations.ai "kontext" img2img  — fallback, real img2img
 *  4. Pollinations.ai text2img           — fallback, ignores the source image
 *  5. prexzyvilla DALL·E                — fallback
 *  6. prexzyvilla Realistic              — fallback
 *
 ...
 * URL, then call each img2img provider with that exact URL so the model
 * conditions on the real image instead of inventing a new one.
 */
...
// ─── Provider 0: David Cyril "NanoBanana" img2img — PRIMARY ────────────────
// Real img2img edit using Google's Gemini "Nano Banana" model via David
// Cyril's free wrapper API (https://apis.davidcyril.name.ng/nanobanana).
//
// NOTE: the live docs page for this endpoint returns a bot-detection block
// when fetched programmatically, and probing the bare endpoint (even a
// plain prompt-only request) returns a 400 with no inspectable body — so
// the exact param names/response shape couldn't be confirmed ahead of time.
// This follows the same GET + query-param convention already used for the
// other David Cyril endpoint in this codebase (commands/media/play.js) and
// for the prexzyvilla img2img call below, and tries a couple of likely
// param spellings before giving up. Response parsing accepts EITHER raw
// image bytes OR a JSON body with the result URL in any common field name
// (via the same extractBestUrl() walker used for prexzyvilla), so it keeps
// working even if the exact JSON shape differs from what's guessed here.
// If this provider's contract turns out to be wrong, it simply returns
// null and the existing fallback chain below takes over — the command
// never breaks because of it.
async function tryNanoBanana(imageBuffer, prompt) {
  try {
    ...
    for (const params of paramVariants) {
      try {
        ...
        if (response.status >= 400) continue; // wrong param name — try next variant
        ...
        // Case 2: API returned JSON — walk it for any usable result URL.
        let json = null;
        ...
        if (json) {
          ...
          continue; // valid JSON but no usable url in it — try next variant
        }

        // Case 3: unlabeled binary (no content-type, not JSON) — accept if
        // it looks like real image data rather than an error stub.
        if (raw.length > 2048) return raw;
      } catch (err) {
        console.error('[editimage] NanoBanana variant failed:', err.message);
      }
    }
    return null;
  } catch (err) {
...
// ─── Provider 1: prexzyvilla img2img — secondary, real img2img via prompt ──
...
async function tryPrexzyvillaImg2Img(imageBuffer, prompt) {
  try {
    ...
    const { data } = await axios.get('https://apis.prexzyvilla.site/ai/img2img', {
      params: {
        imageUrl,
        prompt,
        aspectRatio: '1:1',
    ...
    console.error('[editimage] prexzyvilla img2img failed:', err.message);
    return null;
  }
}

// ─── Provider 2: Pollinations "kontext" img2img — FALLBACK ──────────────────
// Edits the exact source image (via its temp public URL) according to the
...
// ─── Provider 3: Pollinations text2img (FALLBACK — ignores source image) ────
...
// ─── Provider 5: prexzyvilla Realistic ───────────────────────────────────────
async function tryRealistic(prompt) {
  try {
    const { data } = await axios.get('https://apis.prexzyvilla.site/ai/realistic', {
      params: { prompt },
      timeout: 60000,
    });
    if (!data || data.status !== true) return null;
...
module.exports = {
  ...
  async execute({ sock, msg, args, from, reply, prefix }) {
    ...
    try {
      ...
      // Try providers in order — NanoBanana (real img2img) first, then the
      // other img2img providers, then fallbacks that do NOT use the source
      // image, in case every img2img provider is unavailable.
      let resultBuffer = null;
      let providerUsed = '';
      let editedActualImage = false;

      resultBuffer = await tryNanoBanana(finalImageBuffer, prompt);
      if (resultBuffer) {
        providerUsed = 'NanoBanana (David Cyril)';
      ...
      if (!resultBuffer) {
        ...
        if (resultBuffer) {
          providerUsed = 'prexzyvilla img2img';
          editedActualImage = true;
        }
      }
        ...
        from,
        {
          ...
          caption:
            ...
            (editedActualImage
              ...
              : `⚠️ Fallback used — this is a *new* image, not an edit of yours (all img2img providers were unavailable)\n\n`) +
