 ...
 *  1. Pollinations.ai img2img  — free, no API key, reliable
 *  2. Pollinations.ai text2img — generates from prompt + description fallback
 *  3. prexzyvilla DALL·E       — AI image fallback
 *
 * Fix: replaced dead APIs (api.nyxs.pw, api.siputzx.my.id) with
...
// ─── Provider 2: Pollinations text2img with enhanced prompt ──────────────────
...
// ─── Provider 4: prexzyvilla Realistic ───────────────────────────────────────
...
module.exports = {
  ...
  async execute({ sock, msg, args, from, reply, prefix }) {
    ...
    try {
      ...
      if (resultBuffer) providerUsed = 'Pollinations AI';
      ...
      if (!resultBuffer) {
        ...
        if (resultBuffer) providerUsed = 'DALL·E';
      }

      if (!resultBuffer) {
        resultBuffer = await tryRealistic(prompt);
        if (resultBuffer) providerUsed = 'Realistic AI';
      }
