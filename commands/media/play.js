 ...
 *  2. giftedtech
 *  3. paxsenix
 *  4. ryzendesu search → ytmp3
 *  5. tiklydown
 *
 * NOTE: Prexzyvilla-based strategies (Apple Music search, Spotify download,
 * ytdl) were removed on 2026-06-26 — apis.prexzyvilla.site is confirmed dead
 * (provider's own site reports 0 active endpoints despite docs still listing
 * these routes). They were failing on every single .play call, adding dead
 * weight before ever reaching a working strategy. The one siputzx ytsearch
 * call is now reused instead of being duplicated across two strategies.
 */
...
module.exports = {
    ...
    async execute({ sock, msg, from, args, reply, t }) {
        ...
        const strategies = [
            ...
            // ═══ STRATEGY 1 — siputzx search → ytmp3 ═══
            async () => {
            ...
            // ═══ STRATEGY 2 — giftedtech (original) ═══
            async () => {
                const d = await axios.get(
            ...
            // ═══ STRATEGY 4 — ryzendesu search → ytmp3 (original) ═══
            async () => {
                const s = await axios.get(
            ...
            // ═══ STRATEGY 5 — tiklydown direct ytdl-style (original) ═══
            async () => {
                const d = await axios.get(
