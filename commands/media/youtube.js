...
const strategies = [
    // Strategy 1: davidcyril API (same reliable provider already used by .play for audio)
    async (url) => {
    ...
    // Strategy 2: agatz.xyz (same family already used as a fallback in .play)
    async (url) => {
        const { data } = await axios.get(
    ...
    // Strategy 3: prexzyapis (original provider — kept as a further fallback)
    async (url) => {
        const { data } = await axios.get(
