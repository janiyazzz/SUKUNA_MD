...
module.exports = {
    ...
    async execute({ sock, msg, from, reply, args, isOwner }) {
        ...
        // Factor D (device/key-index) is the strongest signal:
        //   devices === []        → key material destroyed → BANNED
        //   devices === [items]   → real identity keys → ACTIVE
        //   devices === null      → probe failed, fall back to A+B
        if (devices !== null) {
            if (devices.length > 0 && page?.ok) {
                // Real devices exist AND (name visible OR profile-hidden)
                result = {
                    emoji: '🟢', status: 'UNBANNED — ACTIVE',
                    detail: `+${target} has a *LIVE* WhatsApp account\n(_valid identity keys: ${devices.length} device(s) on the servers).`,
                    profile: page.title || null,
                };
            }
            else if (devices.length === 0 && page?.generic) {
                // No key material left AND public profile stripped
                result = {
                    emoji: '🔴', status: 'BANNED / OFF-WHATSAPP',
                    detail: `+${target} has *no* WhatsApp account.\n` +
                        `_Signature: identity keys destroyed on the servers — the account was banned & removed._`,
                    profile: null,
                };
            }
        }
