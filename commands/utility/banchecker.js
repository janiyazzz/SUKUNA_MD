...
module.exports = {
    ...
    async execute({ sock, msg, from, reply, args, isOwner }) {
        ...
        if (!result) {
            ...
            if (registered === true) {
                result = { emoji: '🟡', status: 'LIKELY ACTIVE', detail: `+${target} exists on WhatsApp's servers (public check timed out).`, profile: null };
            }
            else if (registered === false) {
                result = { emoji: '🔴', status: 'BANNED / OFF-WHATSAPP', detail: `+${target} is not on WhatsApp's servers (public check timed out).`, profile: null };
            }
            else {
