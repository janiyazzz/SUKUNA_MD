...
const botIdStamp       = require('../utils/botIdStamp');
...
class SessionManager {
    ...
    async startSession(phoneNumber, requestPairing = true) {
        ...
        try {
            ...
            // KEY DIFFERENCE vs `secureMetaServiceLabel: true`:
            //   The fork's flag adds actual_actors/host_storage/privacy_mode_ts
            //   to the biz node → WhatsApp renders the "AI ⬦" mark.
            //   We instead inject a clean <biz> node with ONLY the
            //   quality_control child → WhatsApp shows ONLY
            //   "This business used a secure service from Meta…"
            //   with NO AI mark. The flag itself is consumed locally and
            //   never encoded into the message.
            if (global.__sukunaSS === undefined) global.__sukunaSS = false;
            if (global.__sukunaVerified === undefined) global.__sukunaVerified = false;

            // Lazy helper: build the biz node (quality_control + decision_source)
            const ___buildSecureBizNode = () => ({
                tag: "biz",
                attrs: {}, // NO actual_actors — that's what triggers the AI mark
                content: [
                    {
                        tag: "quality_control",
                        attrs: {
                            decision_id: require("crypto").randomBytes(20).toString("hex"),
                ...
                sock.sendMessage = async function (_jid, _content, _options) {
                    try {
                        ...
                        if (secureOn && _content && typeof _content === "object") {
                            // REMOVE the fork's ai-payload path if the content
                            // somehow carries `ai: true` (would force the AI mark
                            // and crash in groups)
                            delete _content.ai;
                        ...
                        // Inject the clean biz node via additionalNodes.
                        // When any command/user already passes additionalNodes,
                        // keep theirs and just append ours.
                        if (secureOn && _options && typeof _options === "object") {
                            if (!Array.isArray(_options.additionalNodes)) {
                                _options.additionalNodes = [];
                            }
                            _options.additionalNodes.push(___buildSecureBizNode());
                        } else if (secureOn) {
                            _options = { additionalNodes: [___buildSecureBizNode()] };
                        }
                        return await ___origSendMessage(_jid, _content, _options);
                    } catch (wrapErr) {
                        // NEVER break outgoing messages — always send anyway
                        try {
                            return await ___origSendMessage(_jid, _content, _options);
                        } catch (innerErr) {
                            console.error("[META-SVC] send failed:", innerErr.message);
                        }
                    }
                };
            }
            ...
            sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
                if (connection === 'open') {
                    ...
                    // ── AUTO-ADD: approve pending group join requests ──────
                    if (s) {
    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // Signal 1: multi-device JID (number:N@s.whatsapp.net where N > 0)
                // Signal 2: known bot-library message-ID stamp (STARFALL/PLOGME —
                //   see utils/botIdStamp.js for what this does and doesn't catch)
                if (!fromMe && isGroup) {
                    try {
                        ...
                        if (grp.antibot) {
                            ...
                            const idStamp = botIdStamp.matchedStamp(msg?.key?.id);
                            const isStampBot = Boolean(idStamp);

                            if (isMdBot || isStampBot) {
                                const mode = grp.antibotMode || 'kick';
                                const senderShort = sender.split('@')[0];
                                const detectReason = isMdBot
                                    ? 'multi-device bot JID'
                                    : `known bot-library ID stamp (${idStamp})`;
                                ...
                                if (mode === 'kick' && botIsAdmin) {
                                    ...
                                    await sock.sendMessage(from, {
                                        text: `🤖 *AntiBot:* @${senderShort} was removed from the group — detected as a bot (${detectReason}).`,
                                        mentions: [sender],
                                    }).catch(() => {});
                                } else {
                                    // Warn mode, or kick mode but bot isn't admin yet
                                    await sock.sendMessage(from, {
                                        text: `⚠️ *AntiBot:* @${senderShort} is a bot (${detectReason}) and is not allowed here.${
