                ...
                if (database.getNeuro(phoneNumber) && body.trim()) {
                    try {
                        ...
                        if (mentioned || repliedToBot) {
                            ...
                            const neuroSystem = 
                                'You are NEURO — the Jarvis-level AI core of the SUKUNA MD bot. ' +
                                'You manage, debug, and optimize the bot with absolute precision. ' +
                                'Personality: Concise, elite, technical, and efficient. ' +
                                'Your purpose is to solve issues, not explain them at length. ' +
                                'You have direct access to bot controls (ping, menu, system diagnostics). ' +
                                'Keep replies under 2-3 sentences unless providing code or deep logs. ' +
                                'If asked for status, be sharp and high-impact.';
                                
                            const aiReply = await smartAsk({
                                key: 'neuro:' + phoneNumber + ':' + (isGroup ? from : sender),
                                system: neuroSystem,
                                user: cleanText || 'System status check.',
                            }).catch(() => null);

                            if (aiReply) {
                                // Logic to intercept Jarvis-level commands
                                const lowerReply = aiReply.toLowerCase();
                                if (lowerReply.includes('pinging') || lowerReply.includes('latency')) {
                                ...
                                if (lowerReply.includes('menu') || lowerReply.includes('command list')) {
                                ...
                                await sock.sendMessage(from, { text: '🧠 *NEURO:* Neural link recalibrating. System stable.' }, { quoted: msg });
                            }
                        }
