    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // Detects groupStatusMessageV2 posts made by non-admin members
                // and deletes them instantly. In "kick" mode the poster is also
                // removed from the group. Mirrors antilink speed & robustness.
                if (isGroup && !fromMe && !senderIsOwner) {
                    try {
                        ...
                        if (agsMode && agsMode !== false) {
                            ...
                            // groupStatusMessageV2 = someone posted to the group status feed
                            const isGroupStatusPost =
                                !!rawMsg.groupStatusMessageV2 ||
                                !!(rawMsg.messageContextInfo && rawMsg.groupStatusMessageV2);

                            if (isGroupStatusPost) {
                                // Check if sender is a group admin — admins are always exempt
                                let senderIsGroupAdmin = false;
                                try {
                                    const meta = await sock.groupMetadata(from);
                                    const senderNum = this._normJid(sender);
                                ...
                                if (!senderIsGroupAdmin) {
                                    // Delete the group-status post immediately
                                    try {
                                        await sock.sendMessage(from, {
                                            delete: {
                                                remoteJid:   from,
                                    ...
                                    // Kick mode: remove violator from group
                                    if (agsMode === 'kick') {
                                        try {
                                            await sock.groupParticipantsUpdate(from, [sender], 'remove');
                                        } catch (_) {}
