...
class SessionManager {
    ...
    async handleGroupParticipants(sock, phoneNumber, { id, participants, action, author }) {
        try {
            ...
            // ── Metadata fetch with timeout so a slow WhatsApp response
            //    never stalls the entire welcome/goodbye/antihijack block.
            let meta = null;
            try {
            ...
            const safeParticipants = (participants || []).filter(p => this._normJid(p) !== botJid);
            ...
            // View Channel pill — wrapped in try/catch so a missing export
            // never crashes the handler; banner still sends without the pill.
            let channelCtx = {};
            try {
            ...
            const sendGroupMsg = async (participant, kind, customMsg) => {
                ...
                // Profile pic fetch with timeout — never blocks the send.
                let ppUrl = null;
                try {
