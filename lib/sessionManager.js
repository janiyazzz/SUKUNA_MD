        ...
        try {
            ...
            sock.ev.on('presence.update', (presences) => {
                try {
                    ...
                    console.log('[presence] tracking error:', e.message);
                }
            });
    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // Check if this is a reply to a shazam session
                const shazamCmd = commandManager.getCommand('shazam');
                if (shazamCmd?.handleShazamReply) {
                    if (await shazamCmd.handleShazamReply(sock, msg, replyForCmd)) {
                        return; // Handled as shazam reply
                    }
                }
