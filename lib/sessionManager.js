...
class SessionManager {
    constructor() {
        ...
        // Retrieve vault cache: chatJid → Map<msgId, msgObj>
        // Covers BOTH groups AND DMs for the silent .retrieve command.
        this._retrieveCache = new Map();

        // Group metadata cache: groupJid → { meta, ts }
        // TTL = 60s — avoids hitting the WhatsApp API on every command,
        // which was the main cause of slow/no command responses on fresh deploy.
        this._groupMetaCache = new Map();
        this._GROUP_META_TTL = 60000; // 60 seconds
    ...
    async startSession(phoneNumber, requestPairing = true) {
        ...
        try {
            // Ensure any previously held socket for this number is fully torn
            // down before opening a new one. Lingering sockets are the most
            // common cause of code 440 (connectionReplaced) loops where the
            // bot keeps fighting itself for the same WhatsApp session.
            const _prev = this.sessions.get(phoneNumber);
            if (_prev?.sock) {
                try { _prev.sock.ev.removeAllListeners(); } catch (_) {}
                try { _prev.sock.ws?.close(); } catch (_) {}
                try { _prev.sock.end(); } catch (_) {}
            }
            if (_prev?.keepAliveTimer) { try { clearInterval(_prev.keepAliveTimer); } catch (_) {} }
            ...
            sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
                if (connection === 'open') {
                    const s = this.sessions.get(phoneNumber);
                    if (s) { s.status = 'connected'; s.openedAt = Date.now(); }
                    this._reconnectRetries.delete(phoneNumber);
                    console.log(`[SESSION] Connected: ${phoneNumber}`);

                    setTimeout(() => {
                        const cur = this.sessions.get(phoneNumber);
                        if (cur?.status === 'connected' && cur.openedAt && (Date.now() - cur.openedAt) >= 60000) {
                            if (this._replacedStrikes) this._replacedStrikes.delete(phoneNumber);
                            if (this._logoutStrikes)   this._logoutStrikes.delete(phoneNumber);
                        }
                    }, 60000);
                ...
                if (connection === 'close') {
                    ...
                    if (code === DisconnectReason.loggedOut) {
                        // 401 can be transient (WhatsApp blip right after pairing/restore).
                        // Track how many 401s this session has received without a successful
                        // open between them.
                        if (!this._logoutStrikes) this._logoutStrikes = new Map();
                        const strikes = (this._logoutStrikes.get(phoneNumber) || 0) + 1;
                        this._logoutStrikes.set(phoneNumber, strikes);

                        if (strikes <= 2 && config.sessions.autoReconnect) {
                            // First two 401s → reconnect (backoff already handles timing)
                            console.log(`[SESSION] ${phoneNumber}: logout (401), attempt ${strikes}/2 — reconnecting...`);
                            if (sc) sc.status = 'reconnecting';
                            this._scheduleReconnect(phoneNumber);
                        } else {
                            // Persistent logout — stop trying, but DO NOT delete creds so
                            // the user can fix their SESSION_ID and just restart the bot.
                            console.log(`[SESSION] ${phoneNumber}: persistent logout (401) — stopping. ` +
                                        `Update SESSION_ID env var and restart the panel.`);
                            if (this._reconnectTimers.has(phoneNumber)) {
                                clearTimeout(this._reconnectTimers.get(phoneNumber));
                                this._reconnectTimers.delete(phoneNumber);
                            }
                            this._reconnectRetries.delete(phoneNumber);
                            if (sc?.sock) { try { sc.sock.end(); } catch (_) {} }
                            this.sessions.delete(phoneNumber);
                            this.ownerJIDCache.delete(phoneNumber);
                            this.lidToPhoneCache.delete(phoneNumber);
                            // ⚠ intentionally NOT calling deleteSession / rmSync —
                            // creds.json stays on disk so a restart picks it back up.
                        }
                    } else if (config.sessions.autoReconnect) {
                        // ── code 440 (connectionReplaced) handling ───────────
                        // 440 means another client claimed this WhatsApp
                        // session. Aggressively reconnecting just starts a
                        // ping-pong with the other instance and nothing works.
                        // Track strikes; after 3 consecutive 440s, stop and
                        // tell the operator to close the other instance.
                        if (code === 440) {
                            if (!this._replacedStrikes) this._replacedStrikes = new Map();
                            const openedAt = sc?.openedAt || 0;
                            const stableMs = openedAt ? (Date.now() - openedAt) : 0;
                            const rapid    = !openedAt || stableMs < 60000;
                            const s = rapid ? (this._replacedStrikes.get(phoneNumber) || 0) + 1 : 1;
                            this._replacedStrikes.set(phoneNumber, s);
                            if (s >= 3) {
                                console.log(`[SESSION] ${phoneNumber}: connectionReplaced (440) x${s} in <60s — ` +
                                            `the SESSION_ID you pasted is already in use by another device or another deploy. ` +
                                            `Stopping reconnect. Fix: generate a FRESH SESSION_ID, OR clear SESSION_ID in config.js and pair again with your number, then restart the panel.`);
                                if (this._reconnectTimers.has(phoneNumber)) {
                                    clearTimeout(this._reconnectTimers.get(phoneNumber));
                                    this._reconnectTimers.delete(phoneNumber);
                                }
                                this._reconnectRetries.delete(phoneNumber);
                                this._replacedStrikes.delete(phoneNumber);
                                if (sc?.sock) { try { sc.sock.end(); } catch (_) {} }
                                this.sessions.delete(phoneNumber);
                                return;
                            }
                        } else if (this._replacedStrikes) {
                            this._replacedStrikes.delete(phoneNumber);
                        }
                        if (sc) sc.status = 'reconnecting';
            ...
            // ── Retrieve Vault: silent delete capture (groups + DMs) ──────
            // Captures every deleted message across ALL chats without
            // making any announcement.  Owner retrieves them with .retrieve.
            sock.ev.on('messages.upsert', async ({ messages, type }) => {
                if (type !== 'notify') return;
                ...
                for (const m of messages) {
                    try {
                        ...
                        // ── Cache every real message (groups + DMs) ──────────
                        const isProto = !!m.message?.protocolMessage;
                        const isStub  = !!m.messageStubType;
                        if (!isProto && !isStub && m.message && m.key?.id) {
                            if (!this._retrieveCache.has(jid)) this._retrieveCache.set(jid, new Map());
                            const rc = this._retrieveCache.get(jid);
                            rc.set(m.key.id, m);
                            // Cap per chat to avoid memory growth
                            if (rc.size > 300) rc.delete(rc.keys().next().value);
                        }

                        // ── Detect delete (protocolMessage type 0) ───────────
                        const proto = m.message?.protocolMessage;
                        if (!(proto?.type === 0 && proto?.key)) continue;

                        const delId   = proto.key.id;
                        const rc      = this._retrieveCache.get(jid);
                        const origMsg = rc?.get(delId);
                        if (!origMsg) continue; // not cached — cannot recover
                        ...
                        if (body !== null) {
                            retrieveStore.add(phoneNumber, {
                                id: delId, jid, sender, senderNum, deleter, deleterNum,
                                type: 'text', body, caption: null,
                                mimetype: null, fileName: null, ptt: false,
                                mediaBuffer: null,
                            });
                            rc.delete(delId);
                            continue;
                        ...
                        let stored = false;
                        for (const [msgKey, mtype] of Object.entries(mediaMap)) {
                            const mediaMsg = origMsg.message?.[msgKey];
                            if (!mediaMsg) continue;
                            try {
                                ...
                                for await (const chunk of stream) chunks.push(chunk);
                                const buf = Buffer.concat(chunks);
                                if (buf.length < 100) continue; // invalid
                                retrieveStore.add(phoneNumber, {
                                    id: delId, jid, sender, senderNum, deleter, deleterNum,
                                    type:        mtype,
                                    body:        null,
                                ...
                                stored = true;
                                break;
                            } catch (_) { /* try next media type */ }
                        }

                        // Store stub even if media download failed
                        if (!stored) {
                            const mtype = Object.keys(mediaMap).find(k => origMsg.message?.[k]);
                            if (mtype) {
                                retrieveStore.add(phoneNumber, {
                                    id: delId, jid, sender, senderNum, deleter, deleterNum,
                                    type:        mediaMap[mtype],
                                    body:        null,
                                    caption:     origMsg.message?.[mtype]?.caption || null,
                                    mimetype:    origMsg.message?.[mtype]?.mimetype || null,
                                    fileName:    origMsg.message?.[mtype]?.fileName || null,
                                    ptt:         !!origMsg.message?.[mtype]?.ptt,
                                    mediaBuffer: null, // media expired before we could grab it
                                });
                            }
                        }

                        rc.delete(delId);
                    } catch (e) { console.error('[RETRIEVE VAULT]', e.message); }
                }
            });

            // ── Group participant dedup ───────────────────────────────────────
            // Wraps handleGroupParticipants so both the native event AND the stub
            // fallback below can fire without ever double-welcoming the same person.
            if (!this._participantDedup) this._participantDedup = new Map();
            const _participantDedup = this._participantDedup;
            const _dedupHit = (key) => {
                const now = Date.now();
                if (_participantDedup.size > 500)
                    for (const [k, t] of _participantDedup) if (now - t > 60000) _participantDedup.delete(k);
                const last = _participantDedup.get(key);
                if (last && now - last < 60000) return true;
                _participantDedup.set(key, now);
                return false;
            };
            const _origHandle = this.handleGroupParticipants.bind(this);
            this.handleGroupParticipants = async (s, ph, payload) => {
                try {
                    const ids = (payload?.participants || []).slice().sort().join(',');
                    const key = `${payload?.id}:${payload?.action}:${ids}`;
                    if (_dedupHit(key)) return;
                } catch (_) {}
                return _origHandle(s, ph, payload);
            };

            // ── Native group-participants event ───────────────────────────────
            sock.ev.on('group-participants.update', u => this.handleGroupParticipants(sock, phoneNumber, u));

            // ── Stub-based fallback ───────────────────────────────────────────
            // On many Baileys builds the join/leave is ONLY delivered as a
            // messages.upsert with messageStubType — the native event never fires.
            // This listener catches those stubs so welcome/goodbye/introcard always work.
            sock.ev.on('messages.upsert', async ({ messages, type }) => {
                if (type !== 'notify') return;
                for (const m of messages) {
                    try {
                        ...
                        // Correct WAMessageStubType enum values:
                        // 26 = GROUP_PARTICIPANT_ADD    — admin added someone
                        // 27 = GROUP_PARTICIPANT_REMOVE — admin kicked someone
                        // 28 = GROUP_PARTICIPANT_PROMOTE
                        // 29 = GROUP_PARTICIPANT_DEMOTE
                        // 31 = GROUP_PARTICIPANT_INVITE — joined via invite link
                        // 32 = GROUP_PARTICIPANT_LEAVE  — left on their own
                        const ADD_TYPES    = new Set([26, 31, 'GROUP_PARTICIPANT_ADD', 'GROUP_PARTICIPANT_INVITE', 'GROUP_PARTICIPANT_ADD_REQUEST_JOIN']);
                        const REMOVE_TYPES = new Set([27, 32, 'GROUP_PARTICIPANT_REMOVE', 'GROUP_PARTICIPANT_LEAVE', 'GROUP_PARTICIPANT_KICK']);

                        let action = null;
                        if (ADD_TYPES.has(stub))    action = 'add';
                        if (REMOVE_TYPES.has(stub)) action = 'remove';
                        if (!action) continue;

                        const params = (m.messageStubParameters || []).filter(Boolean);
                        if (!params.length) continue;

                        const author = m.key?.participant || m.participant || null;
                        await this.handleGroupParticipants(sock, phoneNumber, {
                            id: jid, participants: params, action, author,
                        });
                    } catch (e) {
                        console.error('[GROUP-STUB] handler error:', e.message);
                    }
                }
            });
    ...
    // ── Cached group metadata fetch ──────────────────────────────────────────
    // Wraps sock.groupMetadata() with a 60-second in-memory TTL cache.
    // Previously every group command (and many moderation checks) called
    // sock.groupMetadata() directly, which makes a live WhatsApp API call
    // every single time. On fresh deploy or after reconnect, these calls
    // pile up and stall message processing — causing commands to appear
    // unresponsive. The cache means only the FIRST call in a 60s window
    // hits the network; all subsequent calls return instantly from memory.
    async _getGroupMeta(sock, groupId) {
        const now   = Date.now();
        const entry = this._groupMetaCache.get(groupId);
        if (entry && (now - entry.ts) < this._GROUP_META_TTL) return entry.meta;
        // Add a 5-second timeout so a stalled WhatsApp response never
        // freezes the message handler indefinitely.
        const meta = await Promise.race([
            sock.groupMetadata(groupId),
            new Promise((_, rej) => setTimeout(() => rej(new Error('meta timeout')), 5000))
        ]);
        this._groupMetaCache.set(groupId, { meta, ts: now });
        return meta;
    }

    async _isGroupAdmin(sock, groupId, sender) {
        try {
            const meta      = await this._getGroupMeta(sock, groupId);
            ...
            try {
                ...
                if (fromMe) {
                    ...
                    if (isGroup && msg.key.participant?.includes('@lid')) {
                        try {
                            const meta = await this._getGroupMeta(sock, from);
                            if (!this.lidToPhoneCache.has(phoneNumber))
                                this.lidToPhoneCache.set(phoneNumber, new Map());
                            const map = this.lidToPhoneCache.get(phoneNumber);
                            for (const p of meta.participants) {
                ...
                // ── AFK: welcome-back detection ──────────────────────────────────
                // Fires whenever the AFK user sends any message in the group.
                // Removes their AFK flag and sends the welcome-back card.
                // Does NOT `continue` — their message/command still processes normally.
                if (isGroup) {
                    try {
                        const afkMod = require('../commands/group/afk');
                        await afkMod.checkAFK(sock, msg, from, sender);
                    } catch (e) { console.error('[AFK-RETURN]', e.message); }
                }

                // ── AFK: notify tagger if mentioned / quoted user is AFK ─────────
                // Fires for every incoming group message that @tags or quotes someone.
                if (!fromMe && isGroup) {
                    try {
                        const afkMod = require('../commands/group/afk');
                        await afkMod.checkMentionedAFK(sock, msg, from);
                    } catch (e) { console.error('[AFK-MENTION]', e.message); }
                }

                // ── FIX [1]: handle button responses here (removed the duplicate
                //    messages.upsert listener that used to do this separately) ────
                //    Now also covers list replies + interactive (native-flow)
                //    replies produced by the chroma menu (@crysnovax/baileys).
                let buttonResponse =
                    ...
                    null;
                if (!buttonResponse) {
                    const iflow = msg.message?.interactiveResponseMessage
                                      ?.nativeFlowResponseMessage;
                    if (iflow?.paramsJson) {
                        try {
                            const parsed = JSON.parse(iflow.paramsJson);
                            buttonResponse = parsed?.id || parsed?.buttonId || parsed?.button_id || null;
                        } catch (_) {}
                    }
                }
                if (buttonResponse) {
                    // If the button id looks like a real command (starts with
                    // the configured prefix), rewrite the message as a plain
                    // text command so the normal dispatcher below picks it up.
                    if (prefix && buttonResponse.startsWith(prefix)) {
                        body = buttonResponse;
                        // Replace the message payload with a synthetic
                        // conversation so any downstream parser that re-reads
                        // msg.message sees a normal text command.
                        try {
                            msg.message = { conversation: buttonResponse };
                        } catch (_) {}
                    } else {
                ...
                if (isGroup && !senderIsOwner) {
                    try {
                        ...
                        if (cooldown > 0) {
                            ...
                            try {
                                const meta = await this._getGroupMeta(sock, from).catch(() => null);
                                if (meta) {
    ...
    async handleGroupParticipants(sock, phoneNumber, { id, participants, action, author }) {
        // Resolve group + metadata defensively so a single failure never
        // blocks the entire handler.
        let grp;
        try { grp = database.getGroup(id); }
        catch (e) { console.error('[GROUP-PARTICIPANTS] getGroup failed:', e.message); grp = {}; }

        let meta = null;
        try {
            meta = await Promise.race([
                this._getCachedMeta(sock, id),
                new Promise((_, rej) => setTimeout(() => rej(new Error('meta timeout')), 4000))
            ]);
        } catch (e) { console.warn('[GROUP-PARTICIPANTS] metadata fetch failed:', e.message); }

        const groupName   = meta?.subject || 'the group';
        const memberCount = meta?.participants?.length || 0;
        const botJid      = this._normJid(sock.user?.id);

        // Filter out the bot itself — never welcome/goodbye ourselves.
        const targets = (participants || []).filter(p => this._normJid(p) !== botJid);

        const TITLE_BOLD    = '𝙎𝙐𝙆𝙐᳇𝘼';
        const FOOTER_ITALIC = '𝓹𝓪𝓼𝓺𝓾𝓪 𝓿𝓮𝓻𝓲𝓯𝓲𝓮𝓭';
        const DIVIDER       = '━━━━━━━━━━━━━━━━━━━━━';

        const expand = (tpl, participant) => {
        ...
        const buildBanner = (kind, participant, customMsg) => {
            ...
            const heading = kind === 'welcome' ? 'Welcome to' : 'Goodbye from';
            const greeting = kind === 'welcome' ? 'Hello' : 'Farewell';
            const fallback = kind === 'welcome' ? 'Welcome to the group!' : 'We will miss you!';
            const tail = (customMsg && String(customMsg).trim())
                ? expand(customMsg, participant)
                : fallback;
            return (
                `┏━〔 ✦ ${TITLE_BOLD} 〕━\n` +
                `❏┃ ${heading} ${groupName}\n` +
        ...
        // Channel pill context — reuses the canonical helper so the
        // "View Channel" pill on welcome/goodbye matches the one on
        // .menu and textmaker exactly. Best-effort: if the require ever
        // fails, the banner still sends without the pill.
        let channelCtx = {};
        try {
            const { CANONICAL_NEWSLETTER } = require('../utils/newsletterBrand');
            channelCtx = {
                isForwarded: true,
                forwardingScore: 999,
                forwardedNewsletterMessageInfo: CANONICAL_NEWSLETTER,
            };
        } catch (_) { /* optional */ }

        const sendGroupMsg = async (participant, kind, customMsg) => {
            const caption = buildBanner(kind, participant, customMsg);
            const ctxInfo = { ...channelCtx, mentionedJid: [participant] };
            const opts    = { mentions: [participant], contextInfo: ctxInfo };
            // Media variant — include messageContextInfo too (matches
            // chromaMenu.js so the channel pill renders on image sends).
            const mediaOpts = {
                ...opts,
            ...
            try {
                ppUrl = await Promise.race([
                    this.fetchProfilePicture(sock, participant),
                    new Promise((resolve) => setTimeout(() => resolve(null), 4000))
                ]);
            } catch (_) { /* no pp — fall through */ }

            if (ppUrl) {
                try {
                    await sock.sendMessage(id, { image: { url: ppUrl }, caption, ...mediaOpts });
                    console.log(`[${kind.toUpperCase()}] sent (image) to ${participant} in ${groupName}`);
                    return;
                } catch (e) {
                    console.warn(`[${kind.toUpperCase()}] image send failed (${e.message}); falling back to text`);
                }
            }
            try {
                await sock.sendMessage(id, { text: caption, ...opts });
                console.log(`[${kind.toUpperCase()}] sent (text) to ${participant} in ${groupName}`);
            } catch (e) {
                console.error(`[${kind.toUpperCase()}] failed to send to ${participant}:`, e.message);
            }
        };

        // ── Welcome ───────────────────────────────────────────────────
        if (action === 'add' && grp && grp.welcome) {
            const msg = grp.welcomeMessage || grp.customWelcome || '';
            for (const p of targets) {
                try { await sendGroupMsg(p, 'welcome', msg); }
                catch (e) { console.error('[WELCOME] unexpected error:', e.message); }
            }
        } else if (action === 'add') {
            console.log(`[WELCOME] skipped in ${id}: welcome=${!!(grp && grp.welcome)}`);
        }

        // ── Intro Card ────────────────────────────────────────────────
        if (action === 'add' && grp && grp.introcard) {
            try {
                const { sendIntroCard } = require('../commands/admin/introcard');
                for (const p of targets) {
                    try { await sendIntroCard(sock, id, p, meta, grp); }
                    catch (e) { console.error('[introcard] per-participant error:', e.message); }
                }
            } catch (e) {
        ...
        // ── Goodbye ───────────────────────────────────────────────────
        if (action === 'remove' && grp && grp.goodbye) {
            const msg = grp.goodbyeMessage || grp.customGoodbye || '';
            // For removals, mentions still work even if user has left.
            for (const p of (participants || []).filter(p => this._normJid(p) !== botJid)) {
                try { await sendGroupMsg(p, 'goodbye', msg); }
                catch (e) { console.error('[GOODBYE] unexpected error:', e.message); }
            }
        } else if (action === 'remove') {
            console.log(`[GOODBYE] skipped in ${id}: goodbye=${!!(grp && grp.goodbye)}`);
        }

        // ── AntiHijack (isolated try so it can never block the above) ─
        try {
