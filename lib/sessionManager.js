    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // run commands. Instead of silently dropping (which made users
                // think "commands don't work in DM"), we now send a clear
                // one-line notice so the sender knows WHY the bot didn't act.
                ...
                if (!publicAliases.has(commandName)) {
                    ...
                    if (database.getSelfMode(phoneNumber) && !senderIsOwner && !senderIsSudo && !senderIsModPM) {
                        try { await reply('🔒 *Bot is in private mode.* Only the owner, mods, and sudo users can use commands right now.'); } catch (_) {}
                        continue;
                    }
                }
                ...
                try {
                    ...
                    console.error(`[CMD:${commandName}]`, cmdErr.message);
                    // FIX: previously a thrown error here was only caught by the
                    // outer per-message try/catch and logged to console — the
                    // sender (including the owner, typing a command in someone
                    // else's DM) got NO feedback at all, which looked exactly
                    // like "the bot doesn't respond". Now we always tell them.
                    try {
                        await reply(`❌ *${commandName}* failed: ${cmdErr.message}`);
                    } catch (_) { /* if even the reply fails, give up silently */ }
                }
