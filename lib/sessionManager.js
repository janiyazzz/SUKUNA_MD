    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // ── Mention React + Mention Message (owner mention detection) ──
                // Fires when someone in a GROUP tags the bot owner's number.
                // Both features are independent — each fires only if enabled.
                if (!fromMe && isGroup) {
                    try {
                        ...
                        // Build owner JID variants to check against mention list
                        const ownerJidVariants = new Set([
                            `${ownerNumber}@s.whatsapp.net`,
                        ]);
                        // Also include any cached LID variants for this owner
                        const lidCache = this.ownerJIDCache.get(phoneNumber);
                        if (lidCache) lidCache.forEach(j => ownerJidVariants.add(j));

                        const ownerWasMentioned = mentionedJids.some(j => {
                            const norm = String(j).split(':')[0].toLowerCase();
                            for (const v of ownerJidVariants) {
                                if (norm === String(v).split(':')[0].toLowerCase()) return true;
                            }
                            // Fallback: compare phone digits only
                            const jPhone = String(j).split('@')[0].split(':')[0].replace(/\D/g, '');
                            return jPhone === ownerNumber;
                        });
                        ...
                        if (ownerWasMentioned) {
                            ...
                            try {
                                ...
                                if (mMsg?.enabled && mMsg.message) {
                                    const senderPart = msg.key.participant || msg.key.remoteJid;
