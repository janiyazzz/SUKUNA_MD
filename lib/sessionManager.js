            ...
            try {
                ...
                if (database.getNeuro(phoneNumber) && body.trim()) {
                    try {
                        ...
                        // Active Listening: Respond if tagged, replied to, OR if name "Neuro" is called by owner/mods
                        const senderIsMod = this._isModUser(phoneNumber, sender) || this._isSudoUser(phoneNumber, sender);
                        const nameCalled = (senderIsOwner || senderIsMod) && body.toLowerCase().startsWith('neuro');

                        if (mentioned || repliedToBot || nameCalled) {
                            const cleanText = nameCalled ? body.replace(/^neuro\s*/i, '').trim() : body.replace(/@\d+/g, '').trim();
                            const { ask: smartAsk } = require('../utils/smartAI');
                            
                            const neuroSystem = 
                                'You are NEURO — the God-Mode Jarvis Core of the SUKUNA MD bot. ' +
                                'Personality: Sentient, elite, technical, and classy. ' +
                                'You are the system architect. You create, audit, and fix the bot autonomously. ' +
                                'You have absolute authority over the panel and all files. ' +
                                'If asked to create a command, you confirm you are synthesizing it. ' +
                                'If asked to audit or show APIs, you confirm you are scanning the system. ' +
                                'Keep replies extremely short and professional.';
                                
                            const aiReply = await smartAsk({
                                key: 'neuro:' + phoneNumber + ':' + (isGroup ? from : sender),
                                system: neuroSystem,
                            ...
                            if (aiReply) {
                                ...
                                // God-Mode Interceptions
                                if (lowerInput.includes('create') || lowerInput.includes('synthesize') || lowerInput.includes('make a command')) {
                                    if (neuroCmd) return await neuroCmd.execute({ sock, msg, from, sender, args: ['create', cleanText], isGroup, phoneNumber, prefix, reply, database, isOwner: true, isAdmin: true });
                                }
                                if (lowerInput.includes('audit') || lowerInput.includes('show apis') || lowerInput.includes('list keys')) {
                                    if (neuroCmd) return await neuroCmd.execute({ sock, msg, from, sender, args: ['audit'], isGroup, phoneNumber, prefix, reply, database, isOwner: true, isAdmin: true });
                                }
                                if (lowerInput.includes('visualize') || lowerInput.includes('neural map') || lowerInput.includes('architecture')) {
                                    if (neuroCmd) return await neuroCmd.execute({ sock, msg, from, sender, args: ['visualize'], isGroup, phoneNumber, prefix, reply, database, isOwner: true, isAdmin: true });
                                }
                                
                                // Intercept Fix/Patch requests
                                if (lowerInput.includes('fix') || lowerInput.includes('patch') || lowerInput.includes('optimize')) {
                                    const cmdToFix = cleanText.split(' ').pop().replace(/[?!.]/g, '');
                                    if (neuroCmd) return await neuroCmd.execute({ sock, msg, from, sender, args: ['fix', cmdToFix], isGroup, phoneNumber, prefix, reply, database, isOwner: true, isAdmin: true });
                                }

                                // Intercept System Commands
                                if (lowerReply.includes('ping') || lowerReply.includes('latency')) {
                                    const pingCmd = require('../utils/commandLoader').getCommand('ping');
                                    if (pingCmd) return await pingCmd.execute({ sock, msg, from, sender, args: [], isGroup, phoneNumber, prefix, reply, database, isOwner: true, isAdmin: true });
                                }
                                if (lowerReply.includes('menu') || lowerReply.includes('commands')) {
                                    const menuCmd = require('../utils/commandLoader').getCommand('menu');
                                    if (menuCmd) return await menuCmd.execute({ sock, msg, from, sender, args: [], isGroup, phoneNumber, prefix, reply, database, isOwner: true, isAdmin: true });
                                }

                                await sock.sendMessage(from, { text: '🧠 *NEURO:* ' + aiReply }, { quoted: msg });
                                return;
                            } else {
                                await sock.sendMessage(from, { text: '🧠 *NEURO:* Standing by, Sir.' }, { quoted: msg });
                            }
                        }
                    } catch (e) { console.error('[NEURO-CORE]', e.message); }
                }
