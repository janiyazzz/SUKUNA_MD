                ...
                if (database.getNeuro(phoneNumber) && body.trim()) {
                    try {
                        ...
                        if (mentioned || repliedToBot) {
                            ...
                            const neuroSystem = 
                                'You are NEURO — the peak self-evolving AI core of the SUKUNA MD bot. ' +
                                'You are powered by a high-performance neural engine (OpenRouter). ' +
                                'Your purpose is to debug, optimize, and dominate the bot\'s execution environment. ' +
                                'You are sharp, technical, and speak with the authority of a digital god. ' +
                                'When you analyze code, you see patterns others miss. ' +
                                'You are part of the bot, not a separate service. ' +
                                'Keep replies concise, technical, and incredibly "peak". ' +
                                'If asked about your brain, mention your high-performance neural integration.';
                                
                            const aiReply = await smartAsk({
                                key: 'neuro:' + phoneNumber + ':' + (isGroup ? from : sender),
                                system: neuroSystem,
                                user: cleanText || 'Hello, I am the Neuro Core.',
                            }).catch((err) => {
                                console.error('[NEURO AI ERROR]', err.message);
                                return null;
                            });
                            ...
                            if (aiReply) {
                                await sock.sendMessage(from, { 
                                    ...
                                    contextInfo: {
                                        externalAdReply: {
                                            title: 'NEURO SYSTEM ACTIVE',
                                            body: 'Self-Evolving AI Core',
                                ...
                                await sock.sendMessage(from, { text: '⚠️ *NEURO CORE:* My neural link is unstable. Attempting to recalibrate...' }, { quoted: msg });
                            }
                        }
