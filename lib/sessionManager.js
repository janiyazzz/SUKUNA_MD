...
class SessionManager {
    ...
    async startSession(phoneNumber, requestPairing = true) {
        ...
        try {
            ...
            // ── GHOST MODE: monkey-patch receipt + read methods so that when
            // the per-session ghostMode flag is ON, the bot silently swallows
            // every outgoing delivery/read receipt. Sender sees one grey tick.
            // Patches read the DB flag on every call → toggling .ghostmode
            // takes effect instantly without reconnecting.
            try {
                const ghostOn = () => {
                    try { return database.getGhostMode(phoneNumber); } catch { return false; }
                };
                const wrap = (name) => {
                    const orig = sock[name];
                    if (typeof orig !== 'function' || orig.__ghostWrapped) return;
                    const wrapped = async (...args) => {
                        if (ghostOn()) return; // swallow — no receipt leaves the socket
                        return orig.apply(sock, args);
                    };
                    wrapped.__ghostWrapped = true;
                    sock[name] = wrapped;
                };
                wrap('sendReceipt');
                wrap('sendReceipts');
                wrap('readMessages');
            } catch (e) { console.error('[GHOSTMODE patch]', e.message); }
