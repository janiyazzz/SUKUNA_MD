...
class SessionManager {
    ...
    async startSession(phoneNumber, requestPairing = true) {
        ...
        try {
            ...
            // ── Group participant stub fallback ───────────────────────────
            // On many Baileys builds the join/leave is ONLY delivered as a
            // messages.upsert with messageStubType GROUP_PARTICIPANT_ADD/REMOVE
            // and never as a group-participants.update event. Without this,
            // welcome + introcard silently no-op. We dedupe with a 60s TTL
            // set so we never double-fire if both paths arrive.
            if (!this._participantDedup) this._participantDedup = new Map();
            const participantDedup = this._participantDedup;
            const dedupHit = (key) => {
                const now = Date.now();
                // prune
                if (participantDedup.size > 500) {
                    for (const [k, t] of participantDedup) if (now - t > 60000) participantDedup.delete(k);
                }
                const last = participantDedup.get(key);
                if (last && now - last < 60000) return true;
                participantDedup.set(key, now);
                return false;
            };
            // wrap the canonical event so it also dedups
            ...
            sock.ev.on('messages.upsert', async ({ messages, type }) => {
                ...
                for (const m of messages) {
                    try {
                        ...
                        if (!stub) continue;
                        // Baileys stub type numbers (also exported as strings on some versions)
                        // 27 = GROUP_PARTICIPANT_ADD, 28 = GROUP_PARTICIPANT_REMOVE,
                        // 29 = GROUP_PARTICIPANT_PROMOTE, 30 = GROUP_PARTICIPANT_DEMOTE,
                        // 31 = GROUP_PARTICIPANT_INVITE, 32 = GROUP_PARTICIPANT_LEAVE
                        const ADD_TYPES    = new Set([27, 31, 'GROUP_PARTICIPANT_ADD', 'GROUP_PARTICIPANT_INVITE', 'GROUP_PARTICIPANT_ADD_REQUEST_JOIN']);
                        const REMOVE_TYPES = new Set([28, 32, 'GROUP_PARTICIPANT_REMOVE', 'GROUP_PARTICIPANT_LEAVE']);
                        let action = null;
    ...
    async handleGroupParticipants(sock, phoneNumber, { id, participants, action, author }) {
        try {
            console.log(`[GROUP-EVENT] action=${action} id=${id} n=${(participants||[]).length}`);
        } catch (_) {}
