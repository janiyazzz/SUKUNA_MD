            ...
            try {
                ...
                if (!fromMe && isGroup) {
                    try {
                        ...
                        // Helper: does this message mention (tag OR reply-to) a given phone number?
                        // Mirrors afk.js's trigger condition: explicit @-tag OR quoting/replying
                        // to that person's message — not just a literal @-tag.
                        const wasMentioned = (targetPhone) => {
                            ...
                            // Check quoted/replied-to sender
                        ...
                        if (senderPhone !== ownerPhone && wasMentioned(ownerPhone)) {
                            ...
                            // DND (Do Not Disturb) — owner only. Same trigger condition
                            // as above (real @-tag via mentionedJid, never a reply-quote).
                            // Also deletes the tagging message — needs the bot to be a
                            // group admin; best-effort, silently no-ops otherwise.
                            try {
