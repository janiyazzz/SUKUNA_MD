...
class SessionManager {
    ...
    async startSession(phoneNumber, requestPairing = true) {
        ...
        try {
            ...
            // ── SECURE SERVICE LABEL + VERIFIED BADGE ────────────────────
            // When toggled ON via .ss / .verified commands, inject flags into
            // every outgoing message so the @crysnovax/baileys fork adds the
            // biz binary node (verified) and/or bot/AI node (secure label).
                ...
                sock.sendMessage = async function (_jid, _content, _options) {
                    try {
                        if (_content && typeof _content === "object") {
                            // SS/AI label: needs both flags
                            if (global.__sukunaSS) {
                                if (_content.text !== undefined) {
                                    _content.secureMetaServiceLabel = true;
                                    _content.ai = true;
                                } else {
                                    // For media messages, add to top-level
                                    _content.secureMetaServiceLabel = true;
                                    _content.ai = true;
                                }
                            }
                            // Verified badge: needs secureMetaServiceLabel
                            if (global.__sukunaVerified) {
                                if (_content.text !== undefined) {
                                    _content.secureMetaServiceLabel = true;
                                } else {
                                    _content.secureMetaServiceLabel = true;
                        ...
                        console.error("[SS/VERIFIED] wrap error:", wrapErr.message);
                        // Fallback: send without flags
                        return await ___origSendMessage(_jid, _content, _options);
                    }
                };
            }
