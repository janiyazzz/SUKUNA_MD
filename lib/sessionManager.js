    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // Runs AFTER sender/body are resolved so all context is available.
                // Detects when the bot owner's number is tagged in any group message.
                if (!fromMe && isGroup && !senderIsOwner) {
                    try {
                        // Collect contextInfo from every possible message type
                        const m = msg.message || {};
                        const mentionCtx =
                        ...
                        // mentionedJid is the canonical list of @-tagged JIDs
                        const mentionedJids = mentionCtx.mentionedJid || [];

                        // Also scan raw text for @ownerNumber pattern as fallback
                        const fullText =
                            body ||
                        ...
                        // Build all possible owner JID representations
                        const ownerPhone = ownerNumber.replace(/\D/g, '');
                        const ownerJidFull = `${ownerPhone}@s.whatsapp.net`;

                        // Check 1: mentionedJid list contains owner JID
                        const mentionMatch = mentionedJids.some(j => {
                            const jPhone = String(j).split('@')[0].split(':')[0].replace(/\D/g, '');
                            return jPhone === ownerPhone;
                        });

                        // Check 2: raw text contains @ownerNumber (e.g. @2348123456789)
                        const textMatch = fullText.includes(`@${ownerPhone}`);

                        // Check 3: LID-based cached owner JIDs
                        const lidCache = this.ownerJIDCache.get(phoneNumber);
                        const lidMatch = lidCache
                            ? mentionedJids.some(j => {
                                const normJ = String(j).split(':')[0].replace(/\D/g, '');
                                for (const cached of lidCache) {
                                    const normC = String(cached).split(':')[0].replace(/\D/g, '');
                                    if (normJ === normC) return true;
                                }
                                return false;
                              })
                            : false;

                        const ownerWasMentioned = mentionMatch || textMatch || lidMatch;

                        if (ownerWasMentioned) {
                            // ── Mention React ──────────────────────────────────
                            try {
                                const mReact = database.getMentionReact(phoneNumber);
                            ...
                            // ── Mention Message ────────────────────────────────
                            try {
                                const mMsg = database.getMentionMessage(phoneNumber);
                ...
                // ── AntiBot message enforcement ───────────────────────────────
                // Detects bots by message behaviour patterns and flags/kicks them.
                // Runs on every group message — complements the join-event check.
                if (!fromMe && isGroup && !senderIsOwner && body) {
                    try {
                        const grp = database.getGroup(from);
                        if (grp.antibot) {
                            const mode = grp.antibotMode || 'kick';

                            // ── Bot fingerprint signals ─────────────────────────
                            // 1. Multi-device JID pattern (device suffix > 0)
                            const mdMatch = String(sender).match(/^(\d+):(\d+)@s\.whatsapp\.net$/);
                            const isMdBot = mdMatch && parseInt(mdMatch[2], 10) > 0;

                            // 2. Bot command triggers — starts with common bot prefixes
                            const BOT_PREFIXES = ['.', '!', '/', '#', '$', '?', '+', '-'];
                            const bodyTrimmed = body.trim();
                            const startsWithBotPrefix = BOT_PREFIXES.some(p => bodyTrimmed.startsWith(p));

                            // 3. Known bot response patterns
                            const BOT_PATTERNS = [
                                /^(?:prefix|commands?|help|menu|ping|alive|info|start|bot)\b/i,
                                /❒|╭─|╰─|⛧.*sukuna|pasqua.*md/i,
                                /\*(?:prefix|uptime|version|ping|owner)\s*:/i,
                            ];
                            const matchesBotPattern = BOT_PATTERNS.some(r => r.test(bodyTrimmed));

                            const isBot = isMdBot || matchesBotPattern;
                            ...
                            if (isBot) {
                                ...
                                if (!senderIsGroupAdmin) {
                                    ...
                                    // Track warnings per sender
                                    if (!sock._antibotWarnings) sock._antibotWarnings = new Map();
                                    const warnKey = `${from}::${sender}`;
                                    const warns = (sock._antibotWarnings.get(warnKey) || 0) + 1;
                                    sock._antibotWarnings.set(warnKey, warns);

                                    // Check if bot is admin before attempting removal
                                    const botSelf = sock.user?.id;
                                    const botPhone = (botSelf || '').split('@')[0].split(':')[0].replace(/\D/g, '');
                                    ...
                                    if (mode === 'kick' && botIsAdmin && warns >= 2) {
                                        try {
                                            await sock.groupParticipantsUpdate(from, [sender], 'remove');
                                            await sock.sendMessage(from, {
                                                text: `🤖 *AntiBot:* @${senderShort} was removed — detected as a bot.`,
                                                mentions: [sender],
                                            });
                                            sock._antibotWarnings.delete(warnKey);
                                        } catch (kickErr) {
                                            console.error('[ANTIBOT MSG] kick failed:', kickErr.message);
                                        }
                                    } else if (warns === 1) {
                                        // First detection — always warn regardless of mode
                                        try {
                                            await sock.sendMessage(from, {
                                                delete: { remoteJid: from, fromMe: false, id: msg.key.id, participant: sender }
                                            });
                                        ...
                                        await sock.sendMessage(from, {
                                            text: `⚠️ *AntiBot:* @${senderShort} has been flagged as a bot.\n\n_${
                                                mode === 'kick' && botIsAdmin
                                                    ? 'One more detection will result in removal.'
                                                    : 'Bot activity is not allowed in this group.'
                                            }_`,
                                            mentions: [sender],
                                        }).catch(() => {});
                                    }
