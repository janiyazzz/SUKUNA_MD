            ...
            try {
                ...
                if (!fromMe && isGroup) {
                    try {
                        ...
                        // Original/official trigger — explicit @-tag only. Used by
                        // Mention React (unchanged behavior — do not touch).
                        ...
                        // Broadened trigger — @-tag OR reply/quote — used ONLY by
                        // DND and Mention Message, per request to match afk.js's
                        // behavior. Mention React intentionally does NOT use this.
                        const wasMentionedOrReplied = (targetPhone) => {
                            if (wasMentioned(targetPhone)) return true;
                            const tp = String(targetPhone).replace(/\D/g, '');
                            if (quotedParticipant &&
                                String(quotedParticipant).split('@')[0].split(':')[0].replace(/\D/g, '') === tp
                            ) return true;
                            return false;
                        };
                        ...
                        const ownerTagged = senderPhone !== ownerPhone && wasMentioned(ownerPhone);
                        const ownerTaggedOrReplied = senderPhone !== ownerPhone && wasMentionedOrReplied(ownerPhone);
                        ...
                        // Mention Message + DND — broadened trigger (tag OR reply)
                        if (ownerTaggedOrReplied) {
                            // Mention Message — fire-and-forget
                            try {
                            ...
                            // DND (Do Not Disturb) — owner only. Also deletes the
                            // tagging message — needs the bot to be a group admin;
                            // best-effort, silently no-ops otherwise.
                            try {
                                const dnd = database.getDndMode(phoneNumber);
                                if (dnd?.enabled && dnd.message) {
                        ...
                        for (const modJid of modUsers) {
                            ...
                            // Mention React for this mod — narrow trigger only (tag, not reply)
                            if (wasMentioned(modPhone)) {
                            try {
                            ...
                            if (!wasMentionedOrReplied(modPhone)) continue;
