                    ...
                    try {
                        if (database.getAutoRead(phoneNumber)) {
                            ...
                            if (msgId && !readSeen.has(msgId)) {
                                ...
                                // sendReadReceipt needs remoteJid + participant (for groups)
                        ...
                        if (agsMode && agsMode !== false) {
                            ...
                            if (isGroupStatusPost) {
                                ...
                                if (!senderIsGroupAdmin) {
                                    ...
                                    // Attempt 3: sendMessage with protocolMessage type REVOKE
                                    try {
                                        await sock.sendMessage(from, {
                                            protocolMessage: {
                                                key: statusKey,
                                    ...
                                    // Kick mode: remove the violator from the group
                                    if (agsMode === 'kick') {
                                        try {
                                            await sock.groupParticipantsUpdate(from, [sender], 'remove');
                                        } catch (_) {}
