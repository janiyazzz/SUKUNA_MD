...
class SessionManager {
    ...
    async startSession(phoneNumber, requestPairing = true) {
        ...
        try {
            ...
            sock.ev.on('group-participants.update', u => {
                // Fire immediately — no await here so it never blocks other events
                this.handleGroupParticipants(sock, phoneNumber, u).catch(e =>
                    console.error('[sessionManager] group-participants handler error:', e.message)
                );
            });
    ...
    // ── Group participants (welcome / goodbye / introcard / antihijack) ──────
    async handleGroupParticipants(sock, phoneNumber, { id, participants, action, author }) {
        try {
            // Delegate welcome / goodbye / introcard to the dedicated event manager.
            await eventManager.handleGroupParticipantsEvent(sock, phoneNumber, { id, participants, action, author });
