...
class SessionManager {
        ...
        for (const msg of messages) {
            try {
                ...
                if (isGroup && body.trim()) {
                    try {
                        if (database.getChatbot(from)) {
                            ...
                            if (!isCmdG) {
                                const botNumG = this._normJid(sock.user?.id);
                                const ctxG    = msg.message?.extendedTextMessage?.contextInfo || {};
                                const mentioned = (ctxG.mentionedJid || []).some(j => this._normJid(j) === botNumG);
                                const repliedToBot = this._normJid(ctxG.participant) === botNumG;
                ...
                if (!publicAliases.has(commandName)) {
                    ...
                    if (database.getSelfMode(phoneNumber) && !senderIsOwner && !senderIsSudo) {
                        continue; // block everyone except owner / sudo
                    }
                }

                // ── Owner-only guard ──────────────────────────────────────────
                if (command.category === 'owner' && !senderIsOwner) {
    ...
    async handleGroupParticipants(sock, phoneNumber, { id, participants, action, author }) {
        try {
            ...
            const TITLE_BOLD = '𝗦𝗨𝗞𝗨𝗡𝗔 𝗠𝗗';      // bold sans-serif
