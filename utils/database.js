...
class Database {
    ...
    // ── Mention React (per session / owner) ────────────────────────────────
    getMentionReact(phoneNumber) {
        if (!this.data.users[phoneNumber]) this.data.users[phoneNumber] = {};
        return this.data.users[phoneNumber].mentionReact || { enabled: false, emoji: '👀' };
    }

    setMentionReact(phoneNumber, value) {
        if (!this.data.users[phoneNumber]) this.data.users[phoneNumber] = {};
        this.data.users[phoneNumber].mentionReact = value;
        this.save('users');
    }

    // ── Mention Message (per session / owner) ──────────────────────────────
    getMentionMessage(phoneNumber) {
        if (!this.data.users[phoneNumber]) this.data.users[phoneNumber] = {};
        return this.data.users[phoneNumber].mentionMessage || { enabled: false, message: '' };
    }

    setMentionMessage(phoneNumber, value) {
        if (!this.data.users[phoneNumber]) this.data.users[phoneNumber] = {};
        this.data.users[phoneNumber].mentionMessage = value;
        this.save('users');
    }
