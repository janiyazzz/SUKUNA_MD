 ...
 *
 * EXEMPTION: Any code that sets sock.__skipBrand = true before calling
 * sock.sendMessage will have the branding completely skipped for that one
 * call. The flag is auto-cleared after use so it only affects the next send.
 *
 * .menu sets this flag so the menu NEVER shows the "View Channel" pill.
 */
'use strict';
...
/**
 * Check if the sock has the skip-brand flag set.
 * If so, clear it and return true (meaning: skip branding).
 */
function checkAndConsumeSkipFlag(sock) {
    if (sock && sock.__skipBrand) {
...
function brandContent(content, skip) {
    ...
    // If skipping, return content as-is — no branding at all
...
function wrapSocket(sock) {
    ...
    sock.sendMessage = (jid, content, options) => {
        try {
            const skip = checkAndConsumeSkipFlag(sock);
            return originalSend(jid, brandContent(content, skip), options);
        }
    ...
    if (typeof sock.relayMessage === 'function') {
        ...
        sock.relayMessage = (jid, message, options) => {
            try {
                const skip = checkAndConsumeSkipFlag(sock);
                if (!skip) rewriteNewsletterDeep(message);
            } catch (_) {}
